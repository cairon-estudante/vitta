...
O sistema deve permitir que novos pacientes se registrem no aplicativo informando nome, e-mail e senha. O paciente é o único perfil que realiza auto-cadastro. A nutricionista (Jesseane) será cadastrada diretamente no Firebase pelo administrador do sistema.
...
- A disponibilidade é baseada na agenda pré-configurada da nutricionista

**RF05 – Solicitar consulta**  
O sistema deve permitir que o paciente selecione um dia e horário disponível (que não tenha consultas já confirmadas pela nutricionista), informe dados mínimos (nome, idade, contato) e envie uma solicitação de consulta.
...
**RF20 – Cancelar consulta (paciente)**  
O sistema deve permitir que o paciente solicite cancelamento de uma consulta aceita.

**RF21 – Cancelar consulta (nutricionista)**  
O sistema deve permitir que a nutricionista cancele uma consulta aceita, atualizando o status e notificando o paciente.

**RF22 – Mudar status de uma consulta (nutricionista)**  
O sistema deve permitir que a nutricionista aceite posteriormente uma consulta cancelada, atualizando o status e notificando o paciente.
